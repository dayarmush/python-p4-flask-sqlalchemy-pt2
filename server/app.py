...
def index():
    response = make_response(
        f'<h1>Welcome to the pet/owner directory!</h1>',
        200
    )
    return response
...
def pets_by_id(id):
    ...
    if not pet:
        response_body = '<h1>Error 404 no pet found</h1>'
...
def owner_by_id(id):
    ...
    if not owner:
        response_body = '<h1>Error 404 no owner found</h1>'
        response = make_response(response_body, 404)
        return response

    response_body = f'<h1>Information for owner {owner.name}</h1>'
